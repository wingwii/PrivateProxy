...
partial class PrivateProxyServer
{
    ...
    private void PrepareHeadJs(StringBuilder sb)
    {
        sb.AppendLine("<script type=\"text/javascript\" >");

        sb.AppendLine("function EnableLoginBtn(x) {");
        sb.AppendLine("var btn = document.getElementById(\"BtnLogin\");");
        sb.AppendLine("if (btn) { btn.disabled = !x; }");
        sb.AppendLine("}");
        
        sb.AppendLine("function LoginCompleted() {");
        sb.AppendLine("EnableLoginBtn(true);");
        sb.AppendLine("try {");

        sb.AppendLine("var frm = document.getElementById(\"HdnLoginFrame\");");
        sb.AppendLine("var content = (frm.contentWindow || frm.contentDocument);");
        sb.AppendLine("if (content.document) content = content.document;");        
        sb.AppendLine("content = content.documentElement.innerText;");

        sb.AppendLine("var serverRet = JSON.parse(content);");        

        sb.AppendLine("var errCode = serverRet.code;");
        sb.AppendLine("var msg = serverRet.msg;");

        sb.AppendLine("if (0 == errCode) {");
        sb.AppendLine("document.body.innerText = msg;");
        sb.AppendLine("}");
        sb.AppendLine("else {");
        sb.AppendLine("document.getElementById(\"DivErrMsg\").innerText = msg;");
        sb.AppendLine("}");
    ...
    private void PrepareTailJS(StringBuilder sb)
    {
        sb.AppendLine("<script type=\"text/javascript\" >");
        this.PrepareHashFuncJs(sb);
        sb.AppendLine("</script>");

        sb.AppendLine("<script type=\"text/javascript\" >");
        
        sb.AppendLine("function BeforeSubmit() {");
        sb.AppendLine("document.getElementById(\"DivErrMsg\").innerText = \"\"");
        sb.AppendLine("var inp1=document.getElementById(\"Passwd\")");
        sb.AppendLine("var inp2=document.getElementById(\"PasswdSALT\")");
        sb.AppendLine("var inp3=document.getElementById(\"HashedPasswd\")");
        sb.AppendLine("inp3.value = md5(\"" + PasswordSALT1 + "-\" + inp2.value + \"-\" + inp1.value + \"-" + PasswordSALT2 + "\");");
        sb.AppendLine("}");
    ...
    private void PrepareWelcomePage(StringBuilder sb)
    {
        sb.Append("<html><head><title>");
        sb.Append("Private Proxy");
        sb.Append("</title></head><body><div style=\"width:100%;text-align:center;\" ><h1>");
        sb.Append("Please login to use this proxy");
        sb.Append("</h1></body></html>");
    }

    private void PrepareHashFuncJs(StringBuilder sb)
    {
